...
// Execute once every quarter
type Quarterly struct {
}

func (s Quarterly) Evaluate(when *api.RuleWhen, facts *api.RuleFacts, rule *api.Rule) bool {
...
func (s Quarterly) getTrigger() (time.Time, time.Time) {
	now := time.Now()
